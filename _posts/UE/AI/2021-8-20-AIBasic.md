...
把多个状态机归为一层，如下图:状态树的元素组成：State分层嵌套成树、Condition（进入条件）、Transition（过渡条件）和Task（行为）；

![](D:\WindCrazyGithubio\windcrazy123.github.io\styles\images\ai\AIBasic\StateTree.png)
