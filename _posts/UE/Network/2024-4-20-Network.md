...
在多人游戏中，我们主要使用Pawn的复制部分来显示角色并与其他人共享一些信息。一个简单的例子是角色的“血量”。但我们不仅复制“血量”以使其对其他人可见，我们还复制它以便服务器对其拥有权限并且客户端无法作弊。那么你可能会问，这个血量不是玩家各自拥有的变量吗，为什么不放在PlayerState中，因为PlayerState的网络更新较慢，Health变量需要经常处理和进行网络传播放在这里不合适，放在Pawn里做成员变量即可。可以查看[网络同步入门-复制频率](https://www.cnblogs.com/lawliet12/p/17306303.html)或参看下表
...
| **GameState**        |                         |        ✔         |                     |         10.0         |     1.0      |
| **PlayerState**      |                         |        ✔         |                     |         1.0          |     1.0      |
| **PlayerController** |            ✔            |                  |                     |        100.0         |     3.0      |
| **Pawn**             |                         |                  |          ✔          |        100.0         |     3.0      |
| **Actor**            |                         |                  |                     |        100.0         |     1.0      |

了解这些设置的含义：

1. `bOnlyRelevantToOwner`：如果为 true，则此**Actor**仅与其所属客户端相关。如果此标志在游戏过程中发生更改，则所有非所有者通道（属于模拟代理的通道）都需要明确关闭。
2. `bAlwaysRelevant`：始终与所有客户端相关（覆盖`bOnlyRelevantToOwner`）。
3. `bReplicateMovement`：如果为真，则复制移动/位置相关的属性。
4. `NetUpdateFrequency`**：此Actor**将被考虑复制的频率（每秒），用于确定`NextUpdateTime`。
5. `NetPriority`：在低带宽或饱和情况下检查复制时的此**Actor的优先级，优先级越高表示复制的可能性越大。**

> 我强烈建议你阅读官方文档：[详细的 Actor 复制流程的](https://docs.unrealengine.com/4.27/en-US/InteractiveExperiences/Networking/Actors/ReplicationFlow/)内容。
...
> 好像采用TCP协议，忘了在哪看到的，不过还有一篇文章说是通过[可靠UDP实现](https://blog.csdn.net/zzq00zzj/article/details/135711945)
...
struct FMyStruct
{
	...
	UPROPERTY()//如果想要复制这个必须添加UPROPERTY()，不然UHT生成的RPC代码中的参数没有此属性
...
> 参考:[复制时间和顺序保证](https://wizardcell.com/unreal/multiplayer-tips-and-tricks/#152-replication-timing-and-ordering-guarantees)
...
6. RPC传输的参数必须被支持，否则`MulticastRPC`将不会进行多播
...
另外，不满足`NetRelevant`的Multicast不会发送到客户端，参考`UNetDriver::ProcessRemoteFunction`。
...
6. 选择所需的设置后，在蓝图中定义 RPC 功能<span id="ActorOwnership">即可。</span>



### RPC 顺序保证

任何可靠的 RPC 都保证按发送顺序接收和处理，因为它们将被重新发送，直到被确认。但是，无论可靠性如何，不同**Actors**之间的顺序仍然无法保证，并且无法保证在同一帧上调用的 RPC 会在同一帧到达。

在理想世界中，在最佳网络条件下，不可靠的 RPC 没有什么不同，但是一旦出现数据包丢失，不可靠的数据包中就会开始出现间隙，因为如果它们丢失了，它们将不会被重新发送。



## 复制 VS RPC

> 参考：[复制属性与 RPC](https://dev.epicgames.com/community/learning/knowledge-base/D7yR/unreal-engine-replicated-properties-vs-rpcs)

RPC 的流程如下：

- 调用 RPC 函数。
- 参数被复制到参数结构中（由 Unreal Header Tool 生成）。
- 接下来，我们搜索代表 RPC 的 `UFunction` 对象（通过Map查找名称），并将其和参数传递给 `UObject::ProcessEvent`（虚拟函数调用）。
- 在 `ProcessEvent` 中，我们需要找出函数的调用空间（虚拟函数调用来确定它是本地的、远程的、Multicast的还是丢弃的）。
- 如果是远程的，我们将调用 `UObject::CallRemoteFunction`（另一个虚拟函数）。
- 然后我们需要遍历与世界相关的每个可用的 `NetDriver`（通常只有 1 个）并调用 `ProcessRemoteFunction`（另一个虚拟函数）。
- 然后需要解决大量的查找：Actor的连接、Actor的通道（如果不存在，则可能导致创建Actor通道）、被实际调用的函数以及函数的 `RepLayout`（同样，如果不存在，则可能会创建）。
- 接下来，我们将序列化这些属性。
- 此时，如果您使用的是不可靠的多播 RPC，或者我们强制 RPC 排队，则该数据将被缓存，然后稍后与复制的属性数据一起发送。如果我们没有排队，我们将继续尝试立即发送。

属性数据的流程如下所示（请注意，这是用于标准 Net Driver 复制，而不是使用 RepGraph）：

- 在帧结束时，服务器将找出可能需要复制的所有Actors。

- 服务器将优先考虑这些Actors。
- 每个连接将处理这些优先的Actors，复制他们需要的内容。
- 在帧中第一次复制Actor时（或者如果我们强制进行网络比较），我们将遍历所有复制的属性以生成更改列表（字面意思是已更改属性的句柄列表），除非启用了`push model`，在这种情况下只会检查标记为`dirty`的属性。
- 服务器将仅发送已更改的属性。

> 还有一些其他文章让你选择使用哪种方法，参考文章：[Correct stateful replication](https://vorixo.github.io/devtricks/stateful-events-multiplayer/)
...
1、Actor的 *所属连接* 是与Actor的所属玩家控制器关联的连接。所有者和所属连接将确定哪个所连接客户端有权做出更改和调用远程函数。也就是说当我一个客户端拥有`ROLE_AutonomousProxy`权限的Actor在服务器代码里调用RPC函数`UFUNCTION(Reliable, Client)`时，我就可以通过这个Actor所包含的连接，知道应该去众多的客户端的哪一个里面执行这个函数。
...
做一个补充：[Owner如何在RPC中生效](https://zhuanlan.zhihu.com/p/34721113)，我们每次调用RPC函数时，会调用`AActor::GetFunctionCallspace`函数判断当前调用位置，如果是就会通过网络发送RPC。`GetFunctionCallspace`里面会调用`GetNetOwningPlayer`通过Owner找到connection信息。
...
{
// Check if the actor can potentially call remote functions	//UFUNCTION(Client)在 actor 的所属客户端上运行
...
`GetLocalRole()`用于获取本地的Role属性，加上之前提到过用`GetNetMode()`确定Actor具有哪种`NetMode`，那么用起来有什么区别呢，虽然大多数时候你可以选择其中任何一种来获得相同的结果，但我使用它们的方式如下：

- **NetMode**：我在**Actor**的[早期](https://docs.unrealengine.com/en-US/ProgrammingAndScripting/ProgrammingWithCPP/UnrealArchitecture/Actors/ActorLifecycle/#spawning)使用此方法，因为在`AActor::PreInitializeComponents()`调用之前网络角色尚未正确设置。
- **NetRole**：我在调用`AActor::PreInitializeComponents()`（包括）后的任何时间点使用此方法。理论上，此方法比前面那个方法更快，因此在可能的情况下应优先使用。

简而言之，**NetMode**更可靠，而**NetRole**更快。
...
> 第5点需要重点强调一下。我们知道所谓连接，不过就是客户端连接到一个服务器，在维持着这个连接的条件下，我们才能真正的玩“网络游戏”。通常，如果我们想让服务器把某些特定的信息发送给特定的客户端，我们就需要找到服务器与客户端之间的这个连接。这个链接的信息就存储在PlayerController的里面，而这个PlayerController不能是随随便便创建的PlayerController，一定是客户端第一次链接到服务器，服务器同步过来的这个PlayerController（也就是上面的第五点，后面称其为**拥有连接的PlayerController**）。进一步来说，这个Controller里面包含着相关的NetDriver，Connection以及Session信息。
>
> 在客户端上对于任何一个Actor，他可以有连接，也可以无连接。一旦Actor有连接，他的Role（控制权限）就是ROLE_AutonomousProxy，如果没有连接，他的Role（控制权限）就是ROLE_SimulatedProxy 。
>
> 那么对于一个Actor，得到这个连接的判断可以看上面的[6、Actor的相关性与优先级](#ActorOwnership)
...
<iframe src="https://blueprintue.com/render/2_2nduas/" scrolling="no" allowfullscreen style="wigth: 100%; height: 500px;"></iframe>
...
<iframe src="https://blueprintue.com/render/13j9ho9f/" scrolling="no"  allowfullscreen style="wigth: 100%; height: 500px;"></iframe>
...
- [@WizardCells](https://twitter.com/WizardCells)
